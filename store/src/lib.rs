...
impl Default for GameState {
    fn default() -> Self {
		Self { stage: Stage::PreGame, board: [
			PlayerType::Bot,
			PlayerType::Bot,
		], active_player_id: 0, players: HashMap::new(), history: Vec::new() }
	}
}
...
impl GameState {
	pub fn validate(&self, event: &GameEvent) -> bool {
		...
		match event {
			...
			PlayerAttack { player_id, enemy_id } => {
				if !self.players.contains_key(player_id) || !self.players.contains_key(enemy_id) {
					return false;
				}
	...
	pub fn trigger(&mut self, valid_event: &GameEvent) {
		...
		match valid_event {
			...
			PlayerAttack { player_id, enemy_id } => {
				let player_attack = self.get_player_stats(player_id).unwrap().attack;
				let mut enemy = self.get_player(enemy_id).unwrap();

				enemy.base_stats.health -= player_attack - enemy.base_stats.defense;

				self.active_player_id = *enemy_id;
			},
		}
	}
	pub fn get_player_stats(&self, player_id: &PlayerId) -> Option<BaseStats> {
	...
	pub fn get_player(&mut self, player_id: &PlayerId) -> Option<&mut Player> {
	...
	pub fn determine_winner(&self) -> Option<PlayerId>{
		let loser_id = self.players.iter().find(|(_, _player)| _player.base_stats.health <= 0).unwrap().0;
		let winner_id = self.players.keys().find(|player_id| *player_id != loser_id).unwrap();
